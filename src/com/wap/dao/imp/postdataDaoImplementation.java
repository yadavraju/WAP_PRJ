...
public class postdataDaoImplementation implements PostdataDAO {
	...
	public void AddPostdata(PostdataModel postdata) {
		...
		String sql = "INSERT INTO postdata (" + "product_you_offer," + "describe_your_offer," + "product_you_need,"
				+ "describe_your_need," + "like_count," + "comment_count," + "longitude," + "latitude,"
				+ "userid) VALUES (" + "'" + product_you_offer + "'," + "'" + describe_your_offer + "'," + "'"
				+ product_you_need + "'," + "'" + describe_your_need + "'," + like_count + "," + comment_count + ","
				+ "'" + logi + "'," + "'" + lati + "'," + userid + ")";
	...
	public int IncrementLikes(int userid, int pid) throws SQLException {
		...
		// else {
		// String updateSql = "update userlike set isLike = 1 where userid=" +
		// userid + " and postid=" + pid;
		// dbcon.doUpdate(updateSql);
		//
		// String sql = "update postdata set like_count = like_count+1 where
		// pid=" + pid;
		// dbcon.doUpdate(sql);
		//
		// isLike = 1;
	...
	public List<PostdataModel> PostdataQuery(String queryString) throws SQLException {
		...
		if (queryString.trim().length() > 0) {
			sql = "SELECT * FROM postdata WHERE product_you_offer LIKE '%" + queryString + "%'"
					+ " OR describe_your_offer LIKE '%" + queryString + "%'" + " OR product_you_need LIKE '%"
					+ queryString + "%'" + " OR describe_your_need LIKE '%" + queryString + "%' ORDER BY DATE DESC";
		...
		while (rs.next()) {
			...
			postdata.setComment_count(rs.getInt("comment_count"));
			postdata.setUserid(rs.getInt("userid"));
			String q = "select fname,lname from user where id = '" + rs.getInt("userid") + "'";
			ResultSet rs1 = dbcon.doSelect(q);
			while (rs1.next()) {
				postdata.setFname(rs1.getString("fname"));
				postdata.setLname(rs1.getString("lname"));
			}
